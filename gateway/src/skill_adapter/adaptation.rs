...
use crate::skill_adapter::compatibility::{check_skill_compatibility, judge_rich_compatibility};
...
pub fn build_adaptation_decision(
    ...
    let project_info =
        detect_from_path(project_root)?.context("failed to detect project metadata from path")?;
