...
use lux::ai_log::{self, AiLogCompactResult, AiLogEntry, AiLogFilter, AiWorkStep, RetentionPolicy};
...
fn ac5_work_step_serialization_uses_camel_case() {
    ...
    assert!(
        value.get("stepName").is_some(),
        "missing stepName (camelCase)"
    );
    ...
    assert!(
        value.get("timestampUtc").is_some(),
        "missing timestampUtc (camelCase)"
    );
