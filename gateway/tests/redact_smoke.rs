...
fn redact_secrets_strips_bearer_tokens() {
    ...
    assert!(
        ...
        "expected [REDACTED] marker for Bearer"
    );
    ...
    assert!(
        ...
        "non-sensitive prefix should survive"
    );
    assert!(
        out.contains("keep-this"),
        "trailing safe text should survive"
    );
}
...
fn redact_project_paths_trailing_slash_normalized() {
    ...
    assert!(
        ...
        "path with trailing slash leaked"
    );
...
fn redact_project_paths_in_entry_payload() {
    ...
    assert!(
        !summary.contains("/Users/me"),
        "summary still has absolute path"
    );
    assert!(
        !details.contains("/Users/me"),
        "details still has absolute path"
    );
...
fn gameplay_redaction_masks_all_sensitive_keys() {
    ...
    assert_eq!(
        p["safeField"], "preserved",
        "non-sensitive field was incorrectly redacted"
    );
}
...
fn gameplay_redaction_null_values_left_untouched() {
    ...
    redact_gameplay_sensitive(&mut value);
    assert!(
        value["playerId"].is_null(),
        "null playerId should stay null"
    );
...
fn metadata_integrity_records_field_paths_and_classes() {
    ...
    assert!(
        meta.redacted_fields
            .contains(&"payload.playerId".to_string()),
...
fn metadata_integrity_never_leaks_original_values() {
    ...
    assert!(
        meta_str.contains("playerId")
            || meta_str.contains("summary")
            || meta_str.contains("ipAddress"),
...
fn metadata_integrity_empty_when_nothing_redacted() {
    ...
    assert!(
        meta.redacted_fields.is_empty(),
        "expected empty redacted_fields"
    );
    assert!(
        meta.redaction_classes.is_empty(),
        "expected empty redaction_classes"
    );
}
...
fn metadata_serde_roundtrips() {
    ...
    let deserialized: RedactionMetadata = serde_json::from_str(&serialized).expect("deserialize");
...
fn retention_enforcement_truncates_to_max_lines() {
    ...
    assert_eq!(
        lines.len(),
        2,
        "expected exactly 2 lines after retention, got {}",
        lines.len()
    );
...
fn retention_enforcement_noop_when_under_limit() {
    let path = write_temp_jsonl("retention-under", "{\"n\":1}\n{\"n\":2}\n");
    ...
    assert_eq!(
        content.lines().count(),
        2,
        "should not drop lines when under limit"
    );
...
fn retention_enforcement_max_lines_one_keeps_only_last() {
    let path = write_temp_jsonl("retention-max-one", "{\"n\":1}\n{\"n\":2}\n{\"n\":3}\n");
...
fn retention_age_test_drops_old_entries() {
    ...
    assert!(
        !result.contains("\"n\":1"),
        "old entry n=1 should be dropped by age"
    );
    assert!(
        !result.contains("\"n\":2"),
        "old entry n=2 should be dropped by age"
    );
    assert!(
        result.contains("\"n\":3"),
        "recent entry n=3 should be kept"
    );
    assert!(
        result.contains("\"n\":4"),
        "recent entry n=4 should be kept"
    );
...
fn retention_age_test_keeps_everything_when_age_unlimited() {
    ...
    assert!(
        result.contains("\"n\":1"),
        "old entry kept when no age limit"
    );
    assert!(
        result.contains("\"n\":2"),
        "old entry kept when no age limit"
...
fn retention_age_combined_with_max_lines() {
    ...
    assert!(
        kept[0].contains("\"n\":4"),
        "should keep the most recent line"
    );
...
fn corrupt_line_handling_compact_reports_dropped_invalid() {
    ...
    assert_eq!(
        result.valid_before, 2,
        "2 valid JSON lines before compaction"
    );
    assert_eq!(
        result.invalid_dropped, 2,
        "2 invalid lines should be reported dropped"
    );
    assert_eq!(result.valid_after, 2, "both valid lines retained");
    assert_eq!(
        result.lines_dropped, 2,
        "total dropped = invalid only (under max)"
    );
...
fn mixed_content_test_compact_counts_and_tail() {
    ...
    assert_eq!(
        result.lines_dropped, 7,
        "7 total dropped (3 valid + 4 invalid)"
    );
    ...
    assert!(
        ...
        "1st valid line should be dropped"
    );
...
fn mixed_content_test_all_invalid_produces_empty_result() {
    let path = write_temp_jsonl("all-invalid", "not-json\n{{broken\n===\n\t\n");

    let entries = read_log_entries(&path, &AiLogFilter::default()).expect("read all-invalid");
    assert!(
        entries.is_empty(),
        "all-invalid file should yield 0 entries"
    );
