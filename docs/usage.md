...
This document is a reference-oriented usage guide for LUX. It focuses on concrete interfaces and surfaces, the CLI, HTTP API, WebSocket events, the dashboard, specs, skills, runs, tickets, and verification. It is intentionally more detailed than the project overview.
...
LUX is local-first. Durable project state is owned under `.lux/`.
...
| `lux gui` | Open the dashboard UI. |
| `lux tui` | Start the terminal UI. |
...
The dashboard is a React 19 single-page application.
...
Routing also includes 2 redirects.

## 6. Spec System

LUX uses a 9-domain spec model. The spec is managed via CLI (`lux spec ...`) and API (`/api/lux/spec...`).
...
- ADR-003: `.lux/spec.json`, `.lux/tickets/*.json`, `.lux/run-state.json`, and `.lux/roadmap.json` are separated by domain.
...
| Godot | partial | Detection and bridge install are verified, build is unsupported. See `docs/godot-support.md`. |
| Three.js | planned | Not verified in this repo snapshot. |
