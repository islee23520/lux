...
namespace Linalab.UnityAiBridge.Editor.Ast
{
    public static class UnityAstSerializer
    {
        ...
        {
            var scenePath = go.scene.IsValid() ? go.scene.path : string.Empty;
            return scenePath + BuildHierarchyPath(go.transform);
        }
        ...
        private static UnityCoordinateMapping[] BuildCoordinateMappings(GameObject go)
        {
            if (go == null)
            {
                return new UnityCoordinateMapping[0];
            }

            var mappings = new List<UnityCoordinateMapping>
            {
                UnityCoordinateMapping.Create(UnityCoordinateFrame.World, "meters", "unity_world", go.transform.position),
                UnityCoordinateMapping.Create(UnityCoordinateFrame.Local, "meters", "parent_local", go.transform.localPosition)
            };

            var rectTransform = go.GetComponent<RectTransform>();
            if (rectTransform != null)
            {
                mappings.Add(UnityCoordinateMapping.Create(UnityCoordinateFrame.UiCanvas, "canvas_units", "canvas_bottom_left", rectTransform.anchoredPosition3D));
            }

            var camera = go.GetComponent<Camera>();
            if (camera != null)
            {
                mappings.Add(UnityCoordinateMapping.Create(UnityCoordinateFrame.Screen, "pixels", "screen_bottom_left", Vector3.zero));
                mappings.Add(UnityCoordinateMapping.Create(UnityCoordinateFrame.Viewport, "normalized", "viewport_bottom_left", Vector3.zero));
                mappings.Add(UnityCoordinateMapping.Create(UnityCoordinateFrame.Input, "screen_pixels", "input_screen", Vector3.zero));
            }
