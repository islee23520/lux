...
fn list_sessions_returns_array() {
    ...
    assert_eq!(json.as_array().unwrap().len(), 1, "expected 1 session after creation");
}
...
fn get_session_by_id() {
    ...
    let json = http_auth_get_json(port, &format!("/api/sessions/{session_id}")).expect("get session JSON");
...
fn delete_session_404_for_missing() {
    ...
    assert_eq!(status, Some(404), "DELETE missing session should return 404");
}
...
fn update_session_updates_timestamp() {
    ...
    let updated =
        http_auth_get_json(port, &format!("/api/sessions/{session_id}")).expect("updated session JSON");
    let new_ts = updated["updatedAtUtc"].as_str().expect("new ts");
    assert_ne!(original_ts, new_ts, "updatedAtUtc should change after PUT update");
    assert_eq!(updated["metadata"]["key"], "v2", "metadata should be updated");
}
