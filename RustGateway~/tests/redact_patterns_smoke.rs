...
fn github_pat_false_positive_partial_prefix() {
    ...
    for input in &inputs {
        let out = redact_secrets(input);
        assert_eq!(out, *input, "'{input}' should not be redacted as GitHub PAT");
    }
}
...
fn openai_key_sk_openai_redacted() {
    ...
    let out = redact_secrets(input);
    assert!(out.contains("[REDACTED]"), "sk-openai key must be redacted, got: {out}");
    assert!(!out.contains("sk-openai-"), "sk-openai- prefix must not survive");
}
...
fn openai_key_sk_project_redacted() {
    ...
    let out = redact_secrets(input);
    assert!(out.contains("[REDACTED]"), "sk-project key must be redacted, got: {out}");
    assert!(!out.contains("sk-project-"), "sk-project- prefix must not survive");
}
...
fn openai_key_exact_minimum_length() {
    ...
    let out = redact_secrets(input);
    assert_eq!(out, "[REDACTED]", "minimum-length OpenAI key must be redacted");
}
...
fn openai_key_false_positive_plain_sk() {
    ...
    let out = redact_secrets(input);
    assert_eq!(out, input, "plain sk- key without qualifier should NOT be redacted");
}
